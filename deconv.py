import numpy as np
from general_utils import find_rise_bsln, find_bsln_return
...
def static_to_motion(
    ...
    and `spot` diameters and the `vel`ocity of the spot (unit [mm]). If the
    `rise_start` index is not provided, it will be calculated with
    `utils.find_rise_bsln` (using `**find_rise_kwargs`)."""
...
def poisson_of_release(rng, rate):
    ...
    """
    return rng.poisson(np.clip(rate, 0.0, np.inf))
